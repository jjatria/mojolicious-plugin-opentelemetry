...
subtest 'incoming HTTP request' => sub {
  subtest 'basic request' => sub {
    my $t = init_app();
    $t->app->routes->get('/rte')->to( cb => sub( $c ) { $c->render( status => 200, json => {} ) });
  ...
  describe 'Host / port parsing' => sub {
      my $t = init_app();
      $t->app->routes->get('/static/url', sub ( $c, @ ) { $c->render( text => 'OK' ) });
...
describe 'Mojo::Log and related' => sub {
  subtest 'sets request ID' => sub {
    my $t = init_app();
    my $log_message = 'Informational';
    my @logs;
    $t->app->log->on(message => sub ($log, @args) { push @logs, \@args });
    ...
    is [grep { $_->[2] eq $log_message } @logs],
      [
        [ 'info', "[$span_id]", $log_message ],
      ],
...
describe 'routes' => sub {
  subtest 'route with callback' => sub {
    my $t = init_app();
  ...
  subtest 'route with placeholder' => sub {
    my $t = init_app();
  ...
  subtest 'Root route is handled correctly' => sub {
    my $t = init_app();
...
describe 'end()' => sub {
  subtest 'delayed rendering with returned promise' => sub {
    my $t = init_app();
  ...
  subtest 'delayed rendering with no promise' => sub {
    my $t = init_app();
    $t->app->routes->get('/delayed', sub( $c ) {
      Mojo::IOLoop->next_tick(sub {
        $c->render( status => 200, json => {} );
  ...
  subtest 'delayed exception handling with promise' => sub {
    my $t = init_app();
    $t->app->routes->get('/error', sub( $c ) {
      $c->render_later;
      return Mojo::Promise->new( sub ($resolve, $reject) {
  ...
  subtest 'delayed exception handling without promise' => sub {
